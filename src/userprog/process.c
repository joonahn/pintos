...
start_process (void *file_name_)
{
  ...
  else
  {
    ...
    //Enlarge stack
    ((char *)if_.esp) -= arg_stack_size;
    next_esp = (char *)if_.esp;

    //Fill stack
    for(index = 0;index<arg_count;index++)
    {
      strlcpy(next_esp, arg_ptr[index], strlen(arg_ptr[index]))
      arg_ptr[index] = next_esp;
      next_esp += (strlen(arg_ptr[index]) + 1);
    }

    // Word align
    for(;if_.esp % 4;((char *)if_.esp)--)
    {
      *((char *)if_.esp) = 0;
    }

    //Enlarge stack
    ((char *)if_.esp) -= ((arg_count + 1) * 4);
    next_esp = (char *)if_.esp;
    ...
    *((char**)next_esp) = 
    ...
    //Fill stack with argv pointer
    arg_stack_size = strlen (file_name);
