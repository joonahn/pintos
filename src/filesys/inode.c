  ...
  if (pos <= inode->data.length)
  {
    block_sector_t pos_sector = pos/BLOCK_SECTOR_SIZE;
      ...
      else
      {
        ...
        block_sector_t level2_idx = pos_sector_double % 128;
        //printf("level1_idx:%d\n",level1_idx);
        //printf("level2_idx:%d\n",level2_idx);
...
bool file_growth(block_sector_t sector, off_t length)
{
  ...
  if (disk_inode != NULL)
  {
    off_t start_sector = bytes_to_sectors (disk_inode->length);
...
inode_close (struct inode *inode) 
{
  ...
  if (--inode->open_cnt == 0)
  {
    ...
    if (inode->removed) 
    {
      ...
      off_t i, i_double;
      for(i = bytes_to_sectors(inode->data.length)-1; i > -1 ; i--)
      {
        if(i<DIRECT_SECTOR_NUM)
    ...
    off_t offset) 
{
  ...
  while (size > 0) 
  {
    ...
    else 
    {
      ...
      //printf("else:write at offset %d,sector_idx:%d,inode:%p\n",sector_ofs,sector_idx,inode);
